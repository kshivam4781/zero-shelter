...
브라우저로 여는 파일 하나입니다. 먼저 실행할 명령과 각각이 해결하는 건수, 코딩
에이전트에 그대로 붙여 넣을 프롬프트, 각 항목이 그 자리에 온 점수, 수용했거나 더 이상
보고되지 않는 것까지 담깁니다. 명령과 프롬프트에는 복사 버튼이 있고, 처음 보는 사람을
위해 각 숫자의 뜻을 접힌 설명으로 붙였습니다. 네트워크도 빌드도 없고, 같은
...
`/zero-shelter:baseline`은 무엇을 수용할지 판단하고 수용 목록을 정직하게 유지하고,
